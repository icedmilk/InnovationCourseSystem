...
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
	...
	else if (status.equals("zhengxuan"))
	{
		/*预存Cno*/
		...
		/***********************************执行选课算法**********************************/
			...
			//查询课容量
			ResultSet rsCmax=db.executeQuery("select cmax from course where cno = "+Cno[k]);
			rsCmax.next();
			int cmax = rsCmax.getInt(1);//记录课容量
			
			//查询最大志愿等级
			sqls[0] = "select max(cstatus) from sc where cno = "+Cno[k];
			...
			for(int i=1;i<=sqlNo[0];i++)
			{
				...
				sqls[i]="select count(cstatus) from sc where cno = "+Cno[k]+" and cstatus = "+i;
			...
			for(int i=1;i<=sqlNo[0];i++)
			{
				if (sqlNo[i]<cmax)
				{
					...
					cmax-=sqlNo[i];//剩余量
					//此时更新为选取状态
					String sqlchoose = "update sc set cstatus = 0 where cstatus = "+i+" and cno= "+Cno[k];
					db.executeUpdate(sqlchoose);
					
				}
				else if (sqlNo[i]>cmax)
				{
					//若第i志愿等级的课容量不够了，志愿冲突的人里随机选择
					String sqltemp = "select  * from sc where cstatus = "+ i 
					+" order by rand() limit "+cmax;
					
					ResultSet rstemp = db.executeQuery(sqltemp);
					...
					{
						String sqlchoose = "update sc set cstatus = 0 where sno = "+temp[j]+" and cno = "+Cno[k];
						db.executeUpdate(sqlchoose);
					}
					
					//搜索剩下没选中的学生
					String setNull = "select sno from sc where cstatus != 0 and cno = "+Cno[k];
					ResultSet rstemp2 = db.executeQuery(setNull);
					
					String[] temp2 = new String[100];
					int count2 = 0;
					while (rstemp2.next())
					{
						temp2[count2++] = rstemp2.getString(1);
					}
					
					//更新状态为-1.代表为没选中
					for(int j = 0;j<count2;j++)
					{
						String sqlunchoose = "update sc set cstatus = -1 where sno = "+temp2[j]+" and cno = "+Cno[k];
		...
		{
			String Traverse = 
			"select course.cname,sc.cstatus from course,sc where course.cno = sc.cno and sno = "+SnoSave[m];
		...
		for (int m = 0;m < countSno;m++)
		{
			String updateSql = "update student set sinfo='"+sinfo[m]+"' where sno="+SnoSave[m];
	...
	response.sendRedirect("ChangeStatus.jsp?user="+request.getParameter("sno"));
%>
...
<html>
  <head>
    ...
    <title>My JSP 'DoChange.jsp' starting page</title>
